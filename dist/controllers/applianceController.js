...
import { createApplianceSchema, updateApplianceSchema, applianceParamsSchema, applianceQuerySchema } from '../middleware/validation.js';
...
export class ApplianceController {
    // GET /api/appliances
    getAppliances = asyncHandler(async (req, res, next) => {
        ...
        if (query.search) {
            ...
            whereConditions.push(or(ilike(appliances.name, searchTerm), ilike(appliances.brand, searchTerm), ilike(appliances.model, searchTerm)));
        }
        // Category filter
        if (query.category) {
        ...
        const baseQuery = whereClause
            ...
            : baseQueryBuilder;
        const sortedQuery = baseQuery.orderBy(query.sortOrder === 'desc' ? desc(sortColumn) : asc(sortColumn));
        ...
        const enhancedResults = results.map((appliance) => {
    ...
    getApplianceStats = asyncHandler(async (req, res, next) => {
        ...
        results.forEach((appliance) => {
            ...
            if (appliance.category && appliance.category in stats.byCategory) {
                stats.byCategory[appliance.category]++;
            }
        });
        res.json({
            success: true,
    ...
    getApplianceById = asyncHandler(async (req, res, next) => {
    ...
    // POST /api/appliances
    createAppliance = asyncHandler(async (req, res, next) => {
        const validatedData = createApplianceSchema.parse(req.body);
        // Calculate warranty expiry date
        const warrantyExpiryDate = calculateWarrantyExpiry(validatedData.purchaseDate, validatedData.warrantyMonths);
    ...
    updateAppliance = asyncHandler(async (req, res, next) => {
        const { id } = applianceParamsSchema.parse(req.params);
    ...
    deleteAppliance = asyncHandler(async (req, res, next) => {
        const { id } = applianceParamsSchema.parse(req.params);
        // Check if appliance exists
        const existing = await db.select().from(appliances).where(eq(appliances.id, id)).limit(1);
        if (existing.length === 0) {
